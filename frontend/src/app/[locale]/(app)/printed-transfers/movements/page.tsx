...
import { usePrintedTransferMovements } from "@/hooks/use-printed-transfers";
import { PRINTED_MOVEMENT_KINDS, type PrintedMovementKind } from "@/lib/schemas/printed-transfer";

const ALL = "__all__";

export default function PrintedTransfersMovementsPage() {
  const t = useTranslations("printedTransfers");
  const tSides = useTranslations("printedTransfers.sides");
  const [kind, setKind] = useState<string>(ALL);

  const { data, isPending, isError } = usePrintedTransferMovements({
    kind: kind === ALL ? undefined : (kind as PrintedMovementKind),
    page_size: 100,
  });
      ...
      <div className="overflow-hidden rounded-[14px] border border-[color:var(--orion-line)] bg-[color:var(--orion-surface)]">
        <div className="flex flex-wrap items-center gap-2 border-b border-[color:var(--orion-line-soft)] bg-[color:var(--orion-surface)] px-4 py-3">
          <Select value={kind} onValueChange={setKind}>
            <SelectTrigger
              data-testid="printed-movements-kind-filter"
