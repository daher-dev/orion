...
export default function PrintedTransfersError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[printed-transfers] page error", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-start gap-3 rounded-[14px] border border-[color:var(--orion-line)] bg-[color:var(--orion-surface)] p-6">
      <h2 className="font-serif text-[18px] font-medium text-[color:var(--orion-ink)]">{t("error")}</h2>
